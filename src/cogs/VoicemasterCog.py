...
class VoicemasterCog(commands.Cog):
    ...
    async def setvmmaster(self, ctx, given_channel_id=None):
        is_a_valid_id = True if given_channel_id and given_channel_id.isdigit() and len(given_channel_id) == 18 else False

        if is_a_valid_id:
            is_a_master = db_gateway().get('voicemaster_master', params={'guild_id': ctx.author.guild.id, 'channel_id': given_channel_id})
            is_voice_channel = True if hasattr(self.bot.get_channel(int(given_channel_id)), 'voice_states') else False
            is_a_slave = db_gateway().get('voicemaster_slave', params={'guild_id': ctx.author.guild.id, 'channel_id': given_channel_id})
