...
class TwitchIntegrationCog(commands.Cog):
    ...
    async def addtwitch(self, ctx, twitch_handle=None, announce_channel=None):
        if twitch_handle is not None and announce_channel is not None:
            ...
            if not twitch_in_db:
                # Check user exists
                user_exists = bool(self.twitch_handler.request_user(twitch_handle))
                if user_exists:
                    # Get live status of the channel
                    live_status = bool(self.twitch_handler.request_data([twitch_handle]))
    ...
    async def editcustomtwitch(self, ctx, twitch_handle=None, custom_message=None):
        if twitch_handle is not None and custom_message is not None:
            ...
            if twitch_in_db:
                ...
                db_gateway().update(
                    'twitch_info',
                    set_params={'custom_message': custom_message},
                    where_params={
                        'guild_id': ctx.author.guild.id,
                        'twitch_handle': twitch_handle.lower()
                    }
    ...
    async def removealltwitch(self, ctx):
        db_gateway().delete('twitch_info', where_params={'guild_id': ctx.author.guild.id})
        await ctx.channel.send("Removed all Twitch alerts from this server")

    @commands.command()
    async def getalltwitch(self, ctx):
        returned_val = db_gateway().get('twitch_info', params={'guild_id': ctx.author.guild.id})
        all_handles = "** **\n__**Twitch Alerts**__\n"
    ...
    async def get_and_compare_statuses(self, alert):
        ...
        all_twitch_handles = db_gateway().pure_return('SELECT DISTINCT twitch_handle FROM "twitch_info"')
        if all_twitch_handles:
            # Create list of all twitch handles in the database
            twitch_handle_arr = list(x['twitch_handle'] for x in all_twitch_handles)
            # Create dict consisting of twitch handles and live statuses
            twitch_status_dict = dict()
            all_twitch_statuses = db_gateway().pure_return('SELECT DISTINCT twitch_handle, currently_live FROM "twitch_info"')
            for twitch_user in all_twitch_statuses:
                twitch_status_dict[twitch_user['twitch_handle']] = twitch_user['currently_live']
            # Query Twitch to receive array of all live users
            returned_data = self.twitch_handler.request_data(twitch_handle_arr)
            ...
            for twitch_handle in twitch_handle_arr:
                ...
                handle_live = (next((obj for obj in returned_data if obj['user_name'].lower() == twitch_handle), False))
                print(handle_live)
                if handle_live:
                    # User is live
                    if not twitch_status_dict[f'{twitch_handle}']:
                        ...
                        db_gateway().update(
                            'twitch_info',
                            set_params={'currently_live': True},
                            where_params={'twitch_handle': twitch_handle.lower()}
                        )
                        if alert:
                            # Grab all channels to be alerted
                            all_channels = db_gateway().get('twitch_info', params={'twitch_handle': twitch_handle.lower()})
                            for each in all_channels:
                                # Send alert to specified channel to each['channel_id']
                                custom_message = each['custom_message'].format(
                                    handle=handle_live['user_name'],
                                    game=handle_live['game_name'],
                                    link=f"https://twitch.tv/{handle_live['user_name']}",
                                    title=handle_live['title']
                                ) if each[
                                    'custom_message'
                                ] != '' else f"{handle_live['user_name']} has just gone live with {handle_live['game_name']}, check them out here: https://twitch.tv/{handle_live['user_name']}"
                                await self.bot.get_channel(each['channel_id']).send(custom_message)
                else:
                    # User is not live
                    db_gateway().update(
                        'twitch_info',
                        set_params={'currently_live': False},
                        where_params={'twitch_handle': twitch_handle.lower()}
                    )
...
class TwitchAPIHandler:
    def __init__(self):
        ...
        self.params = {'client_id': self.client_id, 'client_secret': self.client_secret, 'grant_type': 'client_credentials'}
        self.token = None
